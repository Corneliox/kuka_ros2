...
% Style: Clear, accessible, professional academic English (IMRaD)
%
%%%%%%%%%%%%%%%% Springer %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Contactless human-robot interaction allows operators to command industrial robots using spoken words and vision instead of manual teach pendants. This capability keeps operator hands free and speeds up daily manufacturing tasks. In modern factories, rigid six-axis arms like the KUKA KR6 Agilus series are widely used because they offer high payload rigidity, rapid motion speeds, and repeatability within thirty micrometers.

Recently, end-to-end Vision-Language-Action (VLA) foundation models such as RT-2 \cite{rt2_2023}, OpenVLA \cite{openvla2024}, and Octo \cite{octo2024} have shown great promise. These deep neural networks take camera images and text instructions directly as input to predict robot movements. While VLA models work well for general household tasks, deploying them on industrial robot arms creates several major engineering challenges:
\begin{enumerate}
...
The experimental testbed uses a six-axis KUKA KR6 R900-2 Agilus robot with a 6 kg payload capacity and a 901 mm reach. The arm is controlled by a KUKA KRC4 controller running KSS 8.3 \cite{kuka_kss2018}. A custom vacuum suction gripper is attached to the robot flange. The active Tool Center Point (TCP) offset includes the mounting base length and the flexible bellows height:
\begin{equation}
...
The relationship between image pixel coordinates $\tilde{\mathbf{u}} = [u, v, 1]^T$ and table world coordinates $\tilde{\mathbf{x}}_w = [X_w, Y_w, 1]^T$ is expressed by a $3 \times 3$ homography matrix $\mathbf{H}$:
\begin{equation}
...
The Cartesian coordinates on the table surface are obtained by dividing by the scale factor:
\begin{equation}
    X_w = \frac{h_{11}u + h_{12}v + h_{13}}{h_{31}u + h_{32}v + h_{33}}, \quad Y_w = \frac{h_{21}u + h_{22}v + h_{23}}{h_{31}u + h_{32}v + h_{33}}
\end{equation}

Each marker provides two linear equations in matrix form $\mathbf{A} \in \mathbb{R}^{24 \times 9}$:
\begin{equation}
...
The homography matrix $\mathbf{H}$ is solved using Singular Value Decomposition (SVD) and RANSAC outlier filtering to remove perspective distortion:
\begin{equation}
...
Colored target cubes are segmented in the HSV color space using calibrated thresholds:
\begin{equation}
...
After morphological noise filtering, the center $(\bar{u}, \bar{v})$ of the object is computed using image moments:
\begin{equation}
    M_{pq} = \sum_{u} \sum_{v} u^p v^q \mathcal{B}'(u,v), \quad \bar{u} = \frac{M_{10}}{M_{00}}, \quad \bar{v} = \frac{M_{01}}{M_{00}}
\end{equation}
Applying the homography matrix $\mathbf{H}$ to $(\bar{u}, \bar{v})$ gives the final pick coordinates $(X_{\text{pick}}, Y_{\text{pick}}, Z_{\text{pick}})$.

\subsection{Trajectory Planning and Deterministic Motion Execution}
Robot motions are planned in MoveIt 2 \cite{moveit2014} using the Pilz Industrial Motion Planner \cite{pilz_motion2021}. Point-to-point (PTP) trajectories are used for rapid transit between the home position and approach waypoints 120 mm above the target ($Z_{\text{approach}} = Z_{\text{pick}} + \SI{120}{\milli\meter}$). Linear (LIN) motions are strictly enforced during vertical descent and lifting to prevent horizontal collisions with nearby parts. The gripper orientation is held vertically downward using fixed quaternions:
\begin{equation}
